...
The written values in this skill mirror the token files in `lib/theme`. If this skill and `lib/theme` differ, use `lib/theme` as the source of truth for actual colors, spacing, typography, radius, shadows, layout, and responsive values.
...
- Mobile: 4 columns, `16px` gutter, `16-24px` margin range, recommended X margin `16px`, recommended Y margin `24px`, `320-767px` screen width, `100%` content width.
- Tablet: 8 columns, `24px` gutter, `32-48px` X margin range, `24-40px` Y margin range, recommended X margin `40px`, recommended Y margin `32px`, `768-1279px` screen width, `640-960px` content width.
- Desktop: 12 columns, `24-32px` gutter, `64-120px` X margin range, `32-64px` Y margin range, recommended X margin `80px`, recommended Y margin `48px`, `>=1280px` screen width, `1200-1440px` content width.
- Page outer padding:
  - Desktop: `32-64px`.
  - Tablet: `24-32px`.
  - Mobile: `16-24px`.
...
- When using theme tokens, prefer the additive exports in `lib/theme/layout-tokens.ts` and `lib/theme/standard-design-tokens.ts`. Do not replace existing app-level spacing values unless the requested UI task requires a visible standardization pass.
...
- Card elevation from the PDF:
  - Default Card: `0px 2px 8px rgba(0,0,0,0.06)` for standard cards and list items.
  - Hover/Interactive: `0px 4px 12px rgba(0,0,0,0.08)` for hover states and clickable cards.
  - Highlighted: `0px 8px 24px rgba(0,0,0,0.10)` for featured cards.
  - Modal/Floating: `0px 12px 32px rgba(0,0,0,0.12)` for dialogs and overlays.
...
- Use the Default Card elevation for normal operational cards. Reserve Hover/Interactive only for clickable cards, Highlighted only for intentional emphasis, and Modal/Floating for overlays.
...
- Do not change validation contracts or stored data shapes only to match display formatting. Normalize display and entry behavior only when the requested screen owns that input or output.
...
- Use accordions for expandable record detail, mobile table alternatives, grouped form sections, and secondary information that does not need to be visible all at once.
- Keep trigger rows stable at about `40-48px` height, with clear title text and the expand/collapse icon on the right unless a local component already differs.
- Accordion content should use `16-24px` padding, `1px` subtle borders, and `8px` radius when visually framed.
- Do not hide required actions, validation errors, or primary workflow controls inside collapsed accordions unless the current step or section explicitly owns them.
- On mobile, accordions should be full width and should not create horizontal overflow.
...
- Upper bar finalization should keep identity, active navigation, account/profile affordances, and critical actions visible without crowding.
- Bottom bar finalization should prioritize the most common mobile destinations, use stable icon/button sizes, show the active destination clearly, and avoid duplicate actions already present in the active page header.
...
- Stepper labels should be short and should not wrap into overlapping lines. On mobile, prefer the compact current-step summary over shrinking the full desktop stepper.
...
- Empty states may use `Start by Adding a New Item` only when the user can actually add an item from that context.
...
- Portrait reports should preserve the standard header/body/footer hierarchy and avoid compressing tables beyond readability.
- Landscape reports should use the additional width for tables and signatures without changing report content or field labels.

### Generic Words and Copy

Use the PDF's generic terms when copy is not fixed by business requirements or existing feature terminology.

- Navigation/action terms: Home, Dashboard, Menu, Overview, Search, Explore, Categories, Activity, History, Notifications, Profile, Settings, Help, Support, More.
- Form/action terms: Add, Create, Edit, Update, Save, Submit, Cancel, Delete, Remove, Confirm, Continue, Back, Next, Close, Retry, Refresh, Apply, Reset, Download, Upload, Share, View.
- Data display terms: Name, Description, Details, Type, Category, Status, Date, Start Date, End Date, Amount, Quantity, Address, Contact Number, Email, Notes, Remarks, Required, Optional, Total, Summary, Result, Value, Count, Information, Records, List, Table, Recent, Latest, Preview.
- Status terms: Active, Inactive, Pending, Processing, Approved, Rejected, Completed, Failed, Cancelled, Expired, Draft, Published, Archived.
- Feedback terms: Success, Error, Warning, Loading, No Data Available, Something Went Wrong, Changes Saved, Try Again, Action Completed, Connection Lost, Syncing.
- Authentication terms: Sign In, Sign Out, Register, Forgot Password, Verify, Continue as Guest, Remember Me.
- Empty state terms: No Results Found, No Records Available, Start by Adding a New Item, Nothing to Display, Your List Is Empty.
- Keep current BPIS terminology where already established. For approvals in this repository, use Deny/Denied where the workflow has already standardized on it rather than switching back to Reject/Rejected.
